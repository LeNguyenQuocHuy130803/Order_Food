...
export default function FreshDetailPage() {
  ...
  useEffect(() => {
    const fetchFreshDetail = async () => {
      try {
        ...
        const data = await FreshService.getFreshById(Number(freshId));
        setFresh(data);
      } catch (err) {
        const errorMessage =
          err instanceof Error ? err.message : "Có lỗi xảy ra khi tải dữ liệu";
        setError(errorMessage);
        console.error("Error fetching fresh detail:", err);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const totalPrice = fresh.price * quantity;
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center gap-4 py-4 bg-gray-50  rounded-lg">
              ...
              <span className={`ml-auto px-3 py-1 rounded-full text-sm font-semibold ${
                ...
                {fresh.quantity > 0 ? "Có sẵn" : "Hết hàng"}
              </span>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center border border-gray-300 rounded-lg">
                ...
                <button
                  onClick={() =>
                    setQuantity(Math.min(fresh.quantity, quantity + 1))
                  }
                  className="px-4 py-2 text-lg font-bold hover:bg-gray-100 transition-colors"
